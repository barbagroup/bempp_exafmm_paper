#!/bin/bash
#SBATCH --job-name=1A7M_performance_derivative
...
mkdir -p $REFINE
cd $REFINE
/usr/bin/time -v python ${REPRO_PATH}/bempp_pbs/scripts/derivative_ex.py ${REPRO_PATH}/runs/1A7M_performance/config/1A7M_${REFINE}.yml
cd ..
rm -rf $REFINE
