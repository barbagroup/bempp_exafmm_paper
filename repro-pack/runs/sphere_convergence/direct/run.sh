#!/bin/bash
#SBATCH --job-name=sphere_convergence_direct
...
mkdir -p $SIZE
cd $SIZE
/usr/bin/time -v python ${REPRO_PATH}/bempp_pbs/scripts/direct.py ${REPRO_PATH}/runs/sphere_convergence/config/sphere${SIZE}_r4.yml
cd ..
rm -rf $SIZE
