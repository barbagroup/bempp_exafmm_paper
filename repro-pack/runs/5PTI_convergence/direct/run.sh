#!/bin/bash
#SBATCH --job-name=5PTI_convergence_direct
...
mkdir -p $DENSITY
cd $DENSITY
/usr/bin/time -v python ${REPRO_PATH}/bempp_pbs/scripts/direct.py ${REPRO_PATH}/runs/5PTI_convergence/config/5PTI_ed${DENSITY}.yml
cd ..
rm -rf $DENSITY
