"""
Driver code to compute solvation energy of a structure using direct formulation
with piecewise constant element and block-diagonal preconditioner.

Usage: python direct.py [config.yml]
"""
...
A, rhs1, rhs2 = formulations.direct(dirichl_space, neumann_space, assembler, q, x_q)

# assembly system matrix (including preparing preconditioner)
start = time.time()
A_weak_form = A.weak_form()
precond = preconditioners.block_diagonal(dirichl_space, neumann_space, A)
stop = time.time()
