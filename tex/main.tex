...
Implicit-solvent models via the Poisson-Boltzmann (PB) equation provide insights with less computational cost than atomistic models, making large-system studies---at the scale of viruses---accessible to more researchers.
    ...
    The workflow integrates an easy-to-use Python interface with optimized computational kernels written in compiled languages.
    Users can perform PB simulations interactively via Jupyter notebooks, for faster prototyping and analyzing.
    Our results show the capability of the software, confirm code correctness, and assess performance with between 8,000 and 2 million boundary elements.
    Showcasing the power of this interactive computing platform, just a few lines of code are needed to compare two variants of the boundary integral formulation in regards to conditioning.
    Mesh-refinement studies with a spherical geometry as well as with a real biological structure confirm convergence at the expected $1/N$ rate, for $N$ boundary elements, and
    a comparison with the results from the trusted APBS code using various proteins shows agreement.
    ...
    The results were produced with utmost care for reproducibility, and
all code, primary and secondary data are available in persistent repositories
