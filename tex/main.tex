...
Implicit-solvent models via the Poisson-Boltzmann (PB) equation provide insights with less computational cost than atomistic models, making large-system studies---at the scale of viruses---accessible to more researchers.
    Here we present a high-productivity and high-performance PB solver based on Exafmm, a fast multipole method (FMM) library, and Bempp, a Galerkin boundary element method (BEM) package.
    The workflow integrates an easy-to-use Python interface with optimized computational kernels, and
    users can perform simulations interactively via Jupyter notebooks, for faster prototyping.
    ...
    Showcasing the power of this interactive computing platform, in just a few lines of code we study the conditioning of two variants of the boundary integral formulation.
    Mesh-refinement studies with a spherical geometry and with real biological structures confirm convergence as $1/N$, for $N$ boundary elements, and
