...
Implicit-solvent models expressed by the Poisson-Boltzmann (PB) equation can provide insights with less computational cost than full atomistic models, making large-system studies---at the scale of viruses, for example---accessible to more researchers.
    ...
    Bempp-Exafmm tightly integrates an easy-to-use Python interface with well-optimized computational kernels written in compiled languages.
    Thanks to Python's rich ecosystem in scientific computing, users can perform PB simulations interactively via Jupyter notebooks, enabling faster prototyping and analyzing.
    Our results showcase the capability of the software, confirm code correctness, and assess the performance with problem sizes between 8,000 and 2 million boundary elements.
    Showcasing the power of this interactive computing platform, just a few lines of code are needed to compare two variants of the boundary integral formulation in regards to algebraic conditioning.
    ...
    A comparison with the results from the trusted APBS code using various proteins also substantiates the software credibility.
