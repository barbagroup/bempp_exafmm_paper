...
Binding energy calculations with finite difference codes were thoroughly studied by other researchers for a large test set (51 compounds) with several solvers, in particular, adaptive cartesian grid (ACG),~\cite{HarrisBoschitcshFenley2013} DELPHI, MIBPB, PBSA, and APBS.\cite{nguyenAccurateRobustReliable2017}
In these tests, the authors were specially careful regarding mesh refinement and accuracy.
Table \ref{tab:bind} shows $\Delta G^{polar}_{bind}$ for a subset of the 51 compounds computed with Bempp, compared with the finest mesh results reported by Harris {\it et al.}~\cite{HarrisBoschitcshFenley2013} and Nguyen {\it et al.}.~\cite{nguyenAccurateRobustReliable2017}
We can see that our results are within the variability between other codes, indicating correctness of our implementation. 
...
\begin{table*}[]
    ...
    Energies are in units of kcal/mol. The first 4 columns were taken from the work by Nguyen {\it et al.}~\cite{nguyenAccurateRobustReliable2017} and the last column from Harris {\it et al.}~\cite{HarrisBoschitcshFenley2013} }
    \label{tab:bind}
\end{table*}
