...
This section presents six types of results.
...
Third, we provide more evidence of our software correctness via binding energy computations.
Fourth, we demonstrate the performance with problem sizes between 8,000 and 2 million elements, including timings, breakdowns, and computational complexity.
The fifth type of result compares the performance of Bempp and APBS across different accuracy using a medium-sized protein.
...
Binding energy calculations with finite-difference codes were thoroughly studied by other researchers for a large test set (51 compounds) with several solvers, in particular, adaptive cartesian grid-based PB solver (CPB) \cite{HarrisBoschitcshFenley2013}, DELPHI, MIBPB, PBSA, and APBS \cite{nguyenAccurateRobustReliable2017}.
In these tests, the authors were specially careful regarding mesh refinement and accuracy.

We performed the same test with Bempp to compute $\Delta G^{polar}_{bind}$ for a subset of the 51 compounds.
...
Figure \ref{fig:bind} demonstrates that our results are within the variability between other codes, indicating the correctness of our implementation.
The values of each component in Equation \ref{eqn:bind} are available in the supplementary material.

\begin{table*}[]
    \centering
    \begin{tabular}{c|c|c|c|c|c|c}
         & MIBPB & DELPHI & PBSA  & APBS  & Bempp & CPB \\ \hline
...
\begin{figure*}
    \centering
    \includegraphics[width=0.8\textwidth]{binding_energy.pdf}
...
In this sub-section, we investigate the computational performance of Bempp-Exafmm using 1A7M.
%The sphere has a radius of \qty{1}{\angstrom}, and 100 charges are placed inside, representing the atoms in the solute.
...
\begin{table}[]
    ...
    \hline
    \end{tabular}
    \caption{Simulation parameters used in the performance study for 1A7M.}
...
Table \ref{tab:1A7M_time} presents the assembly time, the solution time and the number of iterations to converge in each case for both formulations.
...
\begin{table*}[]
    ...
    7884                                                         & 27.4                                                     & 7.6                                                         & 19.8                                                     & 33            & 31.2                                                     & 11.1                                                        & 20.1                                                     & 12            \\
    32996                                                        & 89.5                                                     & 12.5                                                        & 77.0                                                     & 50            & 72.7                                                     & 24.2                                                        & 48.5                                                     & 11            \\
    133512                                                       & 403.9                                                    & 34.6                                                        & 369.3                                                    & 92            & 197.3                                                    & 69.3                                                        & 128.0                                                    & 11            \\
    536220                                                       & 1570.7                                                   & 126.2                                                       & 1444.5                                                   & 120           & 646.7                                                    & 261.5                                                       & 385.2                                                    & 11            \\
    2148640                                                      & 7107.8                                                   & 500.7                                                       & 6607.1                                                   & 158           & 2424.3                                                   & 1064.4                                                      & 1359.9                                                   & 11           
    \end{tabular}
    \caption{Assembly and solution times of calculating the solvation energy of 1A7M, using the direct and derivative (exterior) formulations.
...
That explains why the direct formulation leads to a shorter solution time (\gmres time), despite a slower convergence, in the smallest case.
...
Figure \ref{fig:1A7M_assembly_time} shows the linear scaling of the assembly time with respect to $N$.
...
We averaged the time spent on 1 Laplace \fmm and 1 Yukawa \fmm respectively using direct formulation, and plotted it with respect to $N$ in Figure \ref{fig:1A7M_fmm}.
...
In the largest case with over 12 million quadrature points, one Laplace \fmm costs $2.3$s and one Yukawa \fmm costs $6.0$s to compute.
...
Figure \ref{fig:1A7M_gmres_direct} and \ref{fig:1A7M_gmres_derivative} show the time breakdown of \gmres in percentages.
In all cases, \fmm evaluations dominate the solution time.
...
We observed a linear space complexity as shown in Figure \ref{fig:1A7M_memory}.
The largest case, with more than $2$ million elements, requires $30$GB for the direct formulation and $44$GB for the derivative formulation.

\begin{figure*}[t]
\centering
    \subfloat[][Assembly time]{\includegraphics[width=0.33\textwidth]{1A7M_assembly_time.pdf}
        \label{fig:1A7M_assembly_time}}
   \subfloat[][Average evaluation time.]{\includegraphics[width=0.33\textwidth]{1A7M_fmm.pdf}
        \label{fig:1A7M_fmm}}
    \subfloat[][Overall memory consumption in GB.]{\includegraphics[width=0.33\textwidth]{1A7M_memory.pdf}
        \label{fig:1A7M_memory}}\\
    \subfloat[][]{\includegraphics[width=0.4\textwidth]{1A7M_gmres_direct.pdf}
        \label{fig:1A7M_gmres_direct}}
  \subfloat[][]{\includegraphics[width=0.4\textwidth]{1A7M_gmres_derivative.pdf}
        \label{fig:1A7M_gmres_derivative}}
    \caption{Performance on 1A7M;
