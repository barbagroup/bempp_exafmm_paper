...
slp_in = laplace.single_layer(space, space,
    ...
    \end{lstlisting}
\end{adjustbox} \\
Setting up the system matrix of the two derivative formulations, corresponding to Equation \ref{eq:juffer} and \ref{eq:lu}, can be done within just a few lines, as shown in the code snippets below.
...
The prefix denotes the type of operators (single-layer, double-layer, hypersingular or adjoint double-layer), while the suffix suggests whether the operator is the equation corresponding to the solute or solvent region.
...
We generated four surface meshes for Bempp with the element density set to 2, 4, 8 and 16 $\si{\angstrom}^{-2}$, respectively.
...
\begin{table*}[]
    ...
    \end{tabular}
    \caption{Performance of Bempp-Exafmm solving 1RCX.}
